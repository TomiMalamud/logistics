...
interface CacheEntry {
   ...
   */
  export const getSaldoFromCache = (id_comprobante: number): number | undefined => {
    const entry = saldoCache[id_comprobante];
    if (!entry) return undefined;
  
    const now = Date.now();
    const isSaldoPositive = entry.saldo > 0;
   ...
   * @param saldo
   */
  export const setSaldoInCache = (id_comprobante: number, saldo: number): void => {
